...
// ---- Guard: prevent double init (e.g. script included twice) ----
if (window.__prLocalAIInitialized) {
  console.warn("[PromptRebel Local AI] Already initialized - skipping.");
} else {
  window.__prLocalAIInitialized = true;
  ...
  // ---- Knowledge base paths (robust for subpages) ----
  // local-ai.js is in: /assets/js/local-ai.js
  // knowledge is in:   /assets/knowledge/*.md
  const KNOWLEDGE_BASE = new URL("../knowledge/", import.meta.url);

  const KNOWLEDGE = {
    about:     new URL("about.md", KNOWLEDGE_BASE).href,
    faq:       new URL("faq.md", KNOWLEDGE_BASE).href,
    licensing: new URL("licensing.md", KNOWLEDGE_BASE).href,
    music:     new URL("music.md", KNOWLEDGE_BASE).href,
    visuals:   new URL("visuals.md", KNOWLEDGE_BASE).href,
    video:     new URL("video.md", KNOWLEDGE_BASE).href,
    tools:     new URL("tools.md", KNOWLEDGE_BASE).href,
    stories:   new URL("stories.md", KNOWLEDGE_BASE).href,
  };
  ...
  async function loadKnowledge(keys) {
    ...
    for (const k of keys) {
      ...
      if (!knowledgeCache.has(k)) {
        ...
        if (!res.ok) {
          throw new Error(`Knowledge file not found: ${k} (${res.status}) @ ${KNOWLEDGE[k]}`);
        }
...
`.trim();
  ...
  async function pickSources(question, engine) {
    // Fallback ohne LLM (falls Router-Call scheitert)
    const ruleFallback = () => {
      ...
      if (q.includes("wer bist du") || q.includes("promptrebel") || q.includes("about")) return ["about", "faq"];
      return ["faq"];
    };
  ...
  panel.innerHTML = `
    ...
    <div class="pr-ai-body">
      <div class="pr-ai-note">
        ...
        Beim ersten Start wird ein Modell geladen (mehrere 100MB+ möglich).
      </div>
  ...
  // Conversation history (ONLY user/assistant)
  const messages = [];
...
Wenn dir Informationen fehlen, sag das offen.

Wichtig:
- Nutze das geladene Wissensmaterial (Markdown-Snippets).
- Erfinde keine Fakten über PromptRebel, wenn sie nicht im Wissen stehen.
- Wenn eine Frage außerhalb des Wissens liegt: "Das weiß ich nicht" + was du stattdessen brauchst.
`.trim();
  ...
  async function startModel() {
    ...
    try {
      ...
      // reset chat once on successful load
      messages.length = 0;
      msgsEl.innerHTML = "";
      greeted = false;
      ...
      console.error(e);
      progressEl.textContent = "Fehler beim Laden des Modells. (WebGPU/Download prüfen)";
    } finally {
      loading = false;
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const sources = await pickSources(text, engine);
      ...
      messages.push({ role: "assistant", content: reply || "(keine Antwort)" });
    } catch (e) {
      console.error(e);
      aiBubble.textContent =
        "Fehler beim Antworten. Prüfe in der Konsole, ob eine .md-Datei nicht gefunden wurde oder der Router-JSON nicht parsebar war.";
    } finally {
      sending = false;
