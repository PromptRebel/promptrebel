...
export function createGame({ assets }) {
  // --- GAME CONSTANTS (aus deinem letzten funktionierenden Stand, nur Firebase entfernt) ---
  const MAX_LEVEL = 10;
  ...
  const GOLD_KILL_GROWTH = 1.05;   // 5% pro Wave (falls du es doch fix willst: 1.00)
  const BOSS_GOLD = 200;           // fix
  const WAVE_BONUS_BASE = 20;      // Wave 1 = 20
  const WAVE_BONUS_GROWTH = 1.05;  // +5% je Wave

  // --- WAVE SPAWN RULES (dein Wunsch) ---
  const WAVE_COUNT_BASE = 10;
  const WAVE_COUNT_GROWTH = 1.15;  // "x%" -> hier wie bisher 15%
  const WAVE_COUNT_MAX = 50;

  const EXTRA_FROM_WAVE = 30;
  const EXTRA_MAX = 20;
  const EXTRA_SPAWN_MULT = 0.60;   // extra spawn schneller (60% vom interval)
  ...
  const state = {
    ...
    // Spawn controller (kein setInterval -> sauberer bei speed changes)
  ...
  function spawnWave() {
    ...
    // spawn intervals (ms) – werden über performance.now() getrieben
  ...
  function checkWaveEnd() {
    if (state.waveActive && state.enemies.length === 0 && state.spawn.mainLeft <= 0 && state.spawn.extraLeft <= 0) {
      ...
      // Wave bonus (Wave 1 = 20, +5% pro Wave)
  ...
  function update(dt) {
    ...
    for (const t of state.towers) {
      ...
      if (t.cooldown <= 0) {
        ...
        if (inRange.length > 0) {
          let target;
          if (t.targetPriority === 'Strongest') {
            target = inRange.reduce((prev, curr) => (curr.hp > prev.hp) ? curr : prev);
          } else {
            target = inRange.reduce((prev, curr) => (curr.traveled > prev.traveled) ? curr : prev);
          }
          ...
          state.projectiles.push({
            ...
            damage: t.damage, aoe: t.aoe, color: t.color,
            slow: t.slow, slowDur: t.slowDur
          });
  ...
  function updateUI() {
    ...
    document.getElementById('btnStart').disabled = state.waveActive;
    document.getElementById('waveProgress').innerText = state.waveActive ? `${state.waveKilled}/${state.waveTotal}` : "Secure";
  ...
  function handleGameOver() {
    state.hp = 0;
    document.getElementById('overlayText').innerText = `Your firewall collapsed at Wave ${state.wave}.`;
    document.getElementById('overlay').classList.remove('hidden');
  }

  function resetGame() {
    state.hp = 10; state.gold = 120; state.wave = 0;
    state.waveActive = false;
    state.enemies = [];
    state.towers = [];
    state.projectiles = [];
  ...
  function refreshCtx(t) {
    ...
    document.getElementById('targetMode').innerText = t.targetPriority;
    document.getElementById('upgradeLabel').innerText = canUp ? `Lvl ${nextLvl}` : 'Maxed';
    document.getElementById('upgradeCost').innerText = canUp ? `${cost}🪙` : '—';
    document.getElementById('sellValue').innerText = `${Math.floor(t.spent * 0.6)}🪙`;
    document.getElementById('btnUpgrade').disabled = !canUp || state.gold < cost;
  }

  // --- Interactions ---
  document.getElementById('btnCancelSel').onclick = () => cancelSelection();
  document.getElementById('btnCloseOverlay').onclick = () => document.getElementById('overlay').classList.add('hidden');
  ...
  document.querySelectorAll('[data-buy]').forEach(btn => btn.onclick = () => {
    ...
    document.getElementById('selIcon').innerText = data.icon; // UI only
    document.getElementById('selName').innerText = data.name;
  ...
  document.getElementById('btnStart').onclick = spawnWave;
  document.getElementById('btnAuto').onclick = () => {
    state.autoStart = !state.autoStart;
    document.getElementById('btnAuto').innerText = state.autoStart ? 'Auto: On' : 'Auto: Off';
  ...
  document.getElementById('btnSpeed').onclick = () => {
    ...
    document.getElementById('btnSpeed').innerText = `${state.speed}x`;
  };
