...
const y = clamp(p0.y + 20, 70, state.h - 70);
  ...
  function getEndAnchor() {
    ...
    const x = clamp(pn.x + 450, 0, state.w - 80);
    const y = clamp(pn.y + 350, 10, state.h - 10);
  ...
  function drawGatesToBackground(ctx2) {
    ...
    if (endImg) {
      const x = clamp(e.x - END_GATE_W * 0.20, 0, state.w - END_GATE_W);
      const y = clamp(e.y - END_GATE_H * 0.20, -END_GATE_H * 0.6, state.h - END_GATE_H);
      setCrisp(ctx2);
